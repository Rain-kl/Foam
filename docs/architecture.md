...
- 后端提供管理员登录、系统健康/版本、可热更新的系统设置，以及 `example` CRUD 作为扩展模板
- 前端是示例画廊（组件页 + 页面模板），登录走真实会话 API；画廊页优先 mock
- 业务产品逻辑已剥离；二次开发从 `example` 复制开始
...
CLI (cmd/foam → cli.Run)
  ...
  → app.New
       → DB open + schema
       → bootstrap admin
       → wire adminauth + example services
       → httpserver.New (Gin)
  → http.Server.Listen
```
...
脚手架不包含已删除的业务网关 / provider 池配置；新业务配置应有明确归属（YAML 启动项 vs 应用内设置）。
