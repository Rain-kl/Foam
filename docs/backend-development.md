...
5. 装配 `adminauth` + `settings` + `example` 等 service
...
字段：`app.display_name`、`frontend.public_api_base_url`。更新使用 `revision` 乐观锁；空值回退到 `config.yaml` / 默认 `Foam`。
- Token：`infra/security`（JWT）
- 会话持久化：relational admin session repo

前端登录页依赖这组 API；不要为画廊页再发明第二套鉴权。
